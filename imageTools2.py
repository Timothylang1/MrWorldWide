...
# Comp - 123
# Lauren Milne
# This file a modified version of imagetools to allow access to different roots and toplevels. We also changed the
# title of the top level from here. Any changes from imagetools will have a comment next to it.
...
def newroot():  # Added new function that creates a new root that can be accessed from both files
    global root
    root = tk.Tk()
    root.withdraw()
    return root

grandmaster = ["access"]  # Created a global variable that both files can accesss
...
class Picture:
    ...
    def show(self):
        ...
        if self.dispWindow is None:
            self.dispWindow = ImageWindow(self.image, self.width, self.height, "Mr. WorldWide") # Changed to Mr. Worldwide so that all toplevels have the same name
        else:
